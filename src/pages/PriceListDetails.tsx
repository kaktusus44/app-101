...
export function PriceListDetails() {
  ...
  return (
    <main className="light-page">
      <div className="mobile-page price-details-page">
        <PageHeader
          title={list.name}
          actions={
            canEdit ? (
        ...
        />
        {topMenuOpen && (
          <div className="context-menu top-context-menu">
            <button onClick={() => setPdfMenuOpen((value) => !value)}>
              <FileOutput />
              Отправить в PDF
              <ChevronRight />
            </button>
            ...
            {pdfMenuOpen && (
              <div className="pdf-submenu">
                <span>Отправить в PDF</span>
                <button onClick={() => exportPriceListPdf(list, "contractor")}>
                  Подрядчику (только себестоимость)
                </button>
                <button onClick={() => exportPriceListPdf(list, "customer")}>
                  Заказчику (только цена)
                </button>
              </div>
            )}
          </div>
        ...
        <PriceSection
          ...
          action={
            canEdit ? (
              <button onClick={() => setEditor("template")}>
                <PlusCircle />
                Шаблон
              </button>
            ) : null
        ...
        >
          {list.templates.map((template) => (
            ...
            >
              ...
              <ChevronRight />
              {entityMenu?.type === "template" &&
                entityMenu.id === template.id && (
                  <div className="context-menu entity-context-menu">
                    <button
                      onClick={() => editTemplate(template.id, template.name)}
                    >
                      <Pencil />
                      Изменить
                    </button>
                    <button
                      onClick={() => {
                        duplicateTemplate(priceListId, template.id);
                        setEntityMenu(null);
                      }}
                    >
                      <Copy />
                    ...
                    <button
                      onClick={() => {
                        setNotice(
                          "Выбор категории для перемещения добавим вместе с вложенными категориями",
                        );
                        setEntityMenu(null);
                      }}
                    >
                    ...
                    <button
                      ...
                      onClick={() => {
                        deleteTemplate(priceListId, template.id);
                        setEntityMenu(null);
                      }}
                    >
                      <Trash2 />
        ...
        <PriceSection
          ...
          action={
            canEdit ? (
              <button
                onClick={() => navigate(`/price-lists/${list.id}/items/new`)}
              >
                <PlusCircle />
                Позиция
              </button>
        ...
        >
          {items.map((item) => {
            ...
            const margin =
              item.price === 0 ? null : (profit / item.price) * 100;
            return (
              <article className="price-item" key={item.id}>
                <div>
                  <strong>{item.name}</strong>
                  <span>
                    Себестоимость <b>{formatNumber(item.cost)}</b>
                  </span>
                  <span>
                    Прибыль{" "}
                    <b className={profit < 0 ? "negative" : ""}>
                      {formatNumber(profit)}
                    </b>
                  </span>
                  <span>
                ...
                {entityMenu?.type === "item" && entityMenu.id === item.id && (
                  <div className="context-menu entity-context-menu item-context-menu">
                    <button
                      onClick={() =>
                        navigate(
                          `/price-lists/${priceListId}/items/${item.id}/edit`,
                        )
                      }
                    >
                      <Pencil />
                    ...
                    <button
                      onClick={() => {
                        duplicateItem(priceListId, item.id);
                        setEntityMenu(null);
                      }}
                    >
                      <Copy />
                    ...
                    <button
                      className="danger-action"
                      onClick={() => {
                        deleteItem(priceListId, item.id);
                        setEntityMenu(null);
                      }}
                    >
                      <Trash2 />
        ...
        {editor && (
          <div className="sheet-backdrop">
            <form className="editor-sheet" onSubmit={saveEntity}>
              <div className="sheet-heading">
                <button type="button" onClick={() => setEditor(null)}>
                  Отменить
                </button>
                <h2>
                  {editor === "category" ? "Новая категория" : "Новый шаблон"}
                </h2>
                <button disabled={!editorName.trim()}>Сохранить</button>
              </div>
              <label>
                Название
                <input
